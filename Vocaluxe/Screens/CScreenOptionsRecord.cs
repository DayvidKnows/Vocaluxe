...
namespace Vocaluxe.Screens {
	public class CScreenOptionsRecord: CMenu {
		...
		private const string _SelectSlideRecordApis = "SelectSlideRecordApis";
		...
		private readonly string[] _StaticEnergyPlayer = { "StaticEnergyPlayer1", "StaticEnergyPlayer2", "StaticEnergyPlayer3", "StaticEnergyPlayer4", "StaticEnergyPlayer5", "StaticEnergyPlayer6" };
		private float[] _ChannelEnergy;

		private Dictionary<int, ReadOnlyCollection<CRecordDevice>> _DevicesPerApi;
		private int _ApiNr;
		...
		public override void Init() {
			...
			_ThemeSelectSlides = new string[] { _SelectSlideRecordApis, _SelectSlideRecordDevices, _SelectSlideRecordPlayer[0], _SelectSlideRecordPlayer[1], _SelectSlideRecordPlayer[2], _SelectSlideRecordPlayer[3], _SelectSlideRecordPlayer[4], _SelectSlideRecordPlayer[5], _SelectSlideDelay };
			...
			_DevicesPerApi = new Dictionary<int, ReadOnlyCollection<CRecordDevice>>();
		}
		...
		public override void OnShow() {
			...
			_SelectSlides[_SelectSlideRecordApis].Clear();
			_SelectSlides[_SelectSlideRecordDevices].Clear();
			_DevicesPerApi.Clear();
			...
			if (_Devices != null) {
				Dictionary<int, List<CRecordDevice>> devicesPerApi = new Dictionary<int, List<CRecordDevice>>();
				...
				foreach (CRecordDevice device in _Devices) {
					...
					if (!devicesPerApi.ContainsKey(device.HostApi)) {
						devicesPerApi.Add(device.HostApi, new List<CRecordDevice>());
					}

					devicesPerApi[device.HostApi].Add(device);
				}
				_SelectSlides[_SelectSlideRecordApis].Selection = _ApiNr;
				_SelectSlides[_SelectSlideRecordDevices].Selection = _DeviceNr;

				int counter = 0;
				foreach (int hostApi in devicesPerApi.Keys) {
					_DevicesPerApi.Add(counter, new ReadOnlyCollection<CRecordDevice>(devicesPerApi[hostApi]));

					string hostApiName = devicesPerApi[hostApi][0].HostApiName;
					_SelectSlides[_SelectSlideRecordApis].AddValue(hostApiName);
		...
		private void _OnApiEvent() {
			if (_SelectSlides[_SelectSlideRecordApis].Selection != _ApiNr) {
				_ApiNr = _SelectSlides[_SelectSlideRecordApis].Selection;
				_UpdateDevices();
		...
		private void _SelectSlideAction() {
			if (_SelectSlides[_SelectSlideRecordApis].Selected)
				_OnApiEvent();
		...
		private void _UpdateDevices() {
			_Devices = _DevicesPerApi[_ApiNr];
			_SelectSlides[_SelectSlideRecordDevices].Clear();
		...
		private bool _CheckMicConfig() {
			...
			foreach (ReadOnlyCollection<CRecordDevice> devices in _DevicesPerApi.Values) {
